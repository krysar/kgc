...
void max7219_init() {
    ...
    gpio_put(PIN_CS1, 1);
    bi_decl(bi_1pin_with_name(PIN_CS1, "SPI CS"));
    gpio_init(PIN_CS2);
    gpio_set_dir(PIN_CS2, GPIO_OUT);
    bi_decl(bi_1pin_with_name(PIN_CS2, "SPI CS"));
...
void display_number(uint8_t chip,uint8_t display, int64_t number) {
    if((display < 0) || (display > 1))
        ...
        uint8_t num_digits = (number == 0) ? 1 : (uint8_t)log10((double)llabs(number)) + 1;
        ...
        if (number > 0) {
            ...
            switch (num_digits) {
                ...
                case 3:
                    ...
                    break;
                default:
                    spi_send_data(chip, REG_DIGIT0 + (display * 4), code_table[digits[0]] + decimal_points[0] * CODE_DP);
                    spi_send_data(chip, REG_DIGIT1 + (display * 4), code_table[digits[1]] + decimal_points[1] * CODE_DP);
                    spi_send_data(chip, REG_DIGIT2 + (display * 4), code_table[digits[2]] + decimal_points[2] * CODE_DP);
                    spi_send_data(chip, REG_DIGIT3 + (display * 4), code_table[digits[3]] + decimal_points[3] * CODE_DP);
            }
